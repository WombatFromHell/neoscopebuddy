...
graph TB
    ...
    A0 --> A2[SystemDetector.find_executable]
    ...
    A7 --> A9[SystemDetector.is_gamescope_active]
...
- `run`: Entry point method that creates Application instance and runs with command-line arguments, includes exception handling
...
- `should_disable_ld_preload_wrap`: Checks if LD_PRELOAD wrapping should be disabled based on environment variables
...
2. **Conflict Resolution**: Mutually exclusive flags like `-f` (fullscreen) vs `-b` (borderless) are handled properly
...
```bash
python3 -m nscb.pyz -p fullscreen -- /bin/mygame                 # Single profile
...
1. **Input Parsing**:
   - Command line arguments are parsed by the `run` method which calls `ProfileManager.parse_profile_args`
